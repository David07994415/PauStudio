// 精選作品
export const TopProducts = [
    { videoId: "uIatCAvom6Q", type: "Event" },
    { videoId: "cbtqPhIQXLg", type: "Commercial" },
    { videoId: "pC2isw9fxXE", type: "Commercial" },
    { videoId: "ypjRJWYOKUg", type: "Corporate" },
    { videoId: "UMB0sR_m8e0", type: "Commercial" },
    { videoId: "9C_U02GYv98", type: "Commercial" },
    { videoId: "mak0U1g6708", type: "Corporate" },
    { videoId: "tdgSLSaztTc", type: "Brand" },
    { videoId: "B7m675DPbEA", type: "Brand" },
    { videoId: "rUgL7fdnpgk", type: "Corporate" },
    { videoId: "o8m7D_rj17g", type: "Corporate" },
    { videoId: "1qtx-oOWijA", type: "Event" }
]

// 作品分類
export const Categories = [
    {
        type: "Commercial", nameEn: "Commercial Film", nameCh: "廣告｜短片", desc: "類型描述", products: [
            "ZlHgTIy-nmY",
            "cbtqPhIQXLg",
            "pC2isw9fxXE",
            "FbdeBAz1kF4",
            "UMB0sR_m8e0",
            "m9R9sHpRTlg",
            "5Fyie_4nUUg",
            "NxKqZ-8LLLM",
            "vEG7LS7TFlI",
            "9C_U02GYv98",
            "OyUye9cmPaY",
            "4cVkHN-qeMw",
            "cgznbUEnjTE",
            "xC4zHS_Tpko",
            "VQDgkc0z2f8",
        ]
    },
    {
        type: "Corporate", nameEn: "Corporate Video", nameCh: "企業｜形象", desc: "類型描述", products: [
            "4BRPpbM2RO8",
            "MRylU58Q8J0",
            "msyEhc0YHBA",
            "mak0U1g6708",
            "m2gKk4EaF7Y",
            "o8m7D_rj17g",
            "aaGsVQHT9xA",
            "2xQZckDMs7Y",
            "YjrimuQSgtc",
            "07xZu9anrko",
            "rUgL7fdnpgk",
            "kfcBb8uQTZs",
            "upKGpdCAXYs",
            "qtTTD_DRo1Y",
            "wptSWdgvgW8",
            "h1xW7DNlaQ8",
            "cn3GytqnOmQ",
            "P7D72iAZ7l0",
            "GEfgxpsgwCA",
            "G8QpZZ2Brt8",
            "Jl09h2tvubU",
        ]
    },
    {
        type: "Brand", nameEn: "Brand & Product Video", nameCh: "品牌｜產品", desc: "類型描述", products: [
            "tdgSLSaztTc",
            "txWScNMBe1A",
            "3IzkWjyBFF4",
            "itgNo2s-o2A",
            "gfvKr6p4D-8",
            "nRabZM_80JU",
            "B7m675DPbEA",
            "Et6JweGRxWA",
            "8n2QQBLb-Q0",
            "PKfx45knUQU",
            "mrXOVMLsE0o",
            "epbptyClHf4",
            "w96Xmk6kdgE",
            "AVSfzVa-Yc4",
            "hnyDBdFWRuM",
        ]
    },
    {
        type: "Event", nameEn: "Event Documentary", nameCh: "活動｜紀錄", desc: "類型描述", products: [
            "uIatCAvom6Q",
            "1qtx-oOWijA",
            "3HxbNU6-QoQ",
            "JA7gsyYYGU8",
            "lhMBBWiC4mY",
            "T9JiwIcdPI0",
            "IHZZp4zVYsc",
            "ODwy5uclZP4",
            "6pZxCAuDMPo",
            "9M_-VBpJpII",
            "n2CeRlOjtkw",
            "g1EKIV-jZPI",
            "kyCaltvGFHE",
            "AXxFTauaZNU",
            "ReJEv58JOpg",
        ]
    },
    {
        type: "Portrait", nameEn: "Portrait & BTS", nameCh: "人物｜側拍", desc: "類型描述", products: [
            "Sx_3k-vnpwI",
            "o5tZGcA94wQ",
            "3qG9XWneQ1g",
            "JBes0QXDs3s",
            "Rru1yM7YNg4",
            "5cNsy0OnwnI",
            "FJnGacDvZzw",
            "Z1Vn206gmcc",
            "RZUVwwkrA90",
        ]
    },
    {
        type: "Animation", nameEn: "Animation & Motion Graphics", nameCh: "動畫｜片頭", desc: "類型描述", products: [
            "KN1AdVgSAio",
            "wvQG5rrvxrE",
            "iL3EAIpo1rQ",
            "4BRPpbM2RO8",
            "2E2mHoscnCc",
            "G8QpZZ2Brt8",
            "Jl09h2tvubU",
            "s2aR4O7suaQ",
            "TaSAokZFjf8",
            "KURX1o9MMis",
            "neozW97MlfQ",
            "BcYTrVFyGhQ",
        ]
    },
]

// 合作品牌1(local)
export const LogoSlider_First = [
    "01_ADT.png",
    "02_金屬中心.png",
    "03_BEAUTY.png",
    "04_CW.png",
    "05_新北市政府.png",
    "06_桃園市政府.png",
    "07_台中市政府2.png",
    "08_經濟部.png",
    "09_科技部.png",
    "10_綠的傢俱.png",
    "11_KKBOX.png",
    "12_Showtime.png",
    "13_mirise.png",
    "14_國科會.png",
    "15_經濟部產業技術司.png",
    "16_經濟部產業發展署(工業局).png",
    "17_南部科學園區.png",
    "18_富邦金控.png",
    "19_Tokyo Electron.png",
    "20_ZACROS.png",
]

// 合作品牌2(global)
export const LogoSlider_Second = [
    "Biotherm.png",
    "BobbiBrown.png",
    "DR.WU.png",
    "ESTEE LAUDER.png",
    "Folli Follie.png",
    "Hennessy.png",
    "Kiehl_s.png",
    "KOSE.png",
    "LOccitane.png",
    "Loreal Paris.png",
    "MAC.png",
    "Marius Fabre.png",
    "Maybelline.png",
    "Neutrogena.png",
    "Puma.png",
    "SAPPORO.png",
    "Shiseido.png",
    "shu umuera.png",
    "SK-II.png",
    "VOGUE.png",
]


